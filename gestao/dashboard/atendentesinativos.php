...
if(mysqli_num_rows($usuarios) == 0){
  ...
  while($ln = mysqli_fetch_assoc($usuarios)){
    ...
    if(!empty($ln["datetime_online"])){
      ...
      try {
        ...
        // Se a última atividade foi ALÉM do tempo permitido, está inativo
        if($minutosTotais >= $minutos_offline){
          $encontrouInativo = true;
          
          // Calcula tempo de inatividade em formato legível
          $dias = $since_start->days;
          $horas = $since_start->h;
          $minutos = $since_start->i;
          
          $tempoInatividade = "";
          if($dias > 0) $tempoInatividade .= $dias . "d ";
          if($horas > 0) $tempoInatividade .= $horas . "h ";
  ...
  if(!$encontrouInativo){
    echo '<p style="padding: 15px; text-align: center; color: #999;">Todos os atendentes estão ativos</p>';
  }
